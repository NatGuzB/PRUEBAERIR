...
//VIDEOVIDEOVIDEO
 // Variables principales
        ...
        window.addEventListener('scroll', () => {
            ...
            const parallax = document.querySelector('.imagen-section');
        ...
        function scrollToProducts() {
            document.querySelector('.cta-button').scrollIntoView({ 
                behavior: 'smooth' 
            });
        }

        // Efecto de hover interactivo en los granos de café
        document.querySelectorAll('.coffee-bean').forEach(bean => {
            bean.addEventListener('mouseenter', () => {
                bean.style.transform = 'scale(1.5) rotate(45deg)';
                bean.style.background = '#A0522D';
            });
            
            bean.addEventListener('mouseleave', () => {
                bean.style.transform = 'scale(1) rotate(0deg)';
                bean.style.background = '#8B4513';
            });
        });
        ...
        document.querySelectorAll('.historia-texto p').forEach(p => {
            observer.observe(p);
        });
          ...
          function toggleTexto(textoId, button) {
            ...
            const textoCompleto = textoContainer.querySelector('.texto-completo');
            const isExpanded = textoContainer.classList.contains('expanded');
            
            if (isExpanded) {
                // Contraer
                textoContainer.classList.remove('expanded');
                textoCompleto.style.display = 'none';
                button.textContent = 'Ver más';
                button.style.background = 'linear-gradient(135deg, #8B4513 0%, #A0522D 100%)';
            } else {
                // Expandir
                textoContainer.classList.add('expanded');
                textoCompleto.style.display = 'block';
        ...
        // Observar todas las tarjetas de historia
        ...
        document.querySelectorAll('.historia-card').forEach(card => {
            card.addEventListener('mouseenter', () => {
                const iconos = card.querySelectorAll('.icono-cafe');
                iconos.forEach(icono => {
                    icono.style.color = 'rgba(139,69,19,0.3)';
            ...
            card.addEventListener('mouseleave', () => {
                const iconos = card.querySelectorAll('.icono-cafe');
                iconos.forEach(icono => {
                    icono.style.color = 'rgba(139,69,19,0.1)';
